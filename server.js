...
const morgan          = require('morgan'); // Sistema de logging (muestra en la consola los request)
const morganjson      = require('morgan-json');
const apiUsers        = require('./api/users'); //Endpoints relacionados al User model

const app = express();
const db  = levelup('./data/cv', {valueEncoding: 'json'});
...
router.get('/', (req, res) => {
  res.json({ name: 'talentFest-api',version: "0.0.1"});
});
